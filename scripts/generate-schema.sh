...
yq -o json documentation/content/en/reference/schema/kptfile/kptfile.yaml \
  ...
  | yq -P -o yaml > "${BUILD_DIR}/kptfile-schema.yaml"
...
jq -s '.[0] * .[1]' scripts/schema-header.json ${BUILD_DIR}/kptfile-schema.json > ${BUILD_DIR}/kptfile-schema.json

mv ${BUILD_DIR}/kptfile-schema.json documentation/content/en/reference/schema/kptfile/kptfile.json
